...
export class ControladorVendedor {
    ...
    async registrarVendedor(request, response) {
        ...
        let vendedores = await objetoServicioVendedor.buscarVendedores()

        try {
            await objetoServicioVendedor.agregarVendedorEnBD(datosVendedor)
            response.status(200).json({
                "mensaje": "Exito agregando una Vendedor",
                "datos": datosVendedor
            })
            ...
            response.status(400).json({
                "mensaje": "Error agregando una Vendedor" + error,
                "datos": null
            })
        }
    }
    ...
    async editarVendedor(request, response) {
        ...
        try {
            ...
            response.status(200).json({
                "mensaje": "Exito editando vendedor " + id,
                "datos": null,
            })
